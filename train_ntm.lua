...
cmd:option('-model', 'ntm', 'lstm, gru or rnn, lstmex or ntm or lstmntm')
-- optimization
...
if string.len(opt.init_from) > 0 then
    ...
    for i=1,opt.seq_length do
      criterions[i] = nn.ClassNLLCriterion()
    end
end
...
function eval_split(split_index, max_batches)
    ...
    for i = 1,n do -- iterate over batches in the split
        ...
        for t=1,opt.seq_length do
            ...
            local prediction = protos.rnn:f(x[{{1}, t}])
            loss = loss +
            criterions[t]:forward(prediction, y[{{1}, t}])
        end
...
function feval(x)
    ...
    local loss = 0
    for t=1,opt.seq_length do
        protos.rnn:training() -- make sure we are in correct mode (this is cheap, sets flag)
        --predictions[t] = protos.rnn:forward(x[{{}, t}])
        predictions[t] = protos.rnn:forward(x[{{1}, t}]):clone()
        --loss = loss + protos.criterions[t]:forward(predictions[t], y[{{}, t}])
        loss = loss + criterions[t]:forward(predictions[t], y[{{1}, t}])
    end
    ...
    for t=opt.seq_length,1,-1 do
        ...
        local grad = criterions[t]:backward(predictions[t], y[{{1}, t}])
        --print(t,grad)
        protos.rnn:backward(x[{{1}, t}], grad)
    end
    ------------------------ misc ----------------------
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/lm_%s_epoch%.3f_%.4f.t7', opt.checkpoint_dir, opt.savefile, epoch, val_loss)
    ...
    if i % opt.print_every == 0 then
        print(string.format("%d/%d (epoch %.3f), train_loss = %6.8f, grad/param norm = %6.4e, time/batch = %.2fs , lr = %f", i, iterations, epoch, train_loss, grad_params:norm() / params:norm(), time, optim_state.learningRate))
    end
